...
obj529986.Source = "obj430811.Position = Vector3.new(0, 0, 0)
obj430811.Anchored = true
obj430811.BrickColor = BrickColor.new(\"Medium stone" ..
" grey\")
...
" true
	...
	for i = 1, 8 do
		...
		if v then
			...
			points[#points" ..
" + 1] = {v, 0, t0, v.WorldPosition, vel + extravel, false};
...
"r(0.925, 0.975)
	...
	points[#points + 1] = {" ..
"sphere.Bone, 0, t0, sphere.Bone.WorldPosition, vel + extravel, false};
...
"= workspace.CurrentCamera
	...
	cumshots[#cumshots + 1] = {
		...
		mainpart = sphere;
		l" ..
"ife = rng:NextNumber(5, 6);
		points = points;
		ignore = ignore;
	}
end

game:GetService('RunService').Heartbeat:Connec" ..
"t(function(dt)
	local j = 1
	for i = 1, #cumshots do
		local v = cumshots[i]
		cumshots[i] = nil
		if not v or not v.cum" ..
" or not v.cum.Parent then
			continue
		end
...
"3] then
				if not y[6] or not y[6].Parent then
					y[6] = nil
					local newvel = y[5] - (Vector3.yAxis * cumgravity *" ..
" dt)
					local dtvel = (newvel + y[5]) * dt / 2
					y[5] = newvel
					local results = workspace:Raycast(y[1].WorldPos" ..
"ition, dtvel, v.ignore) --v.frames < 3 and v.ignore or nil)
					if results then
						if not v.hit then
							v.hit = " ..
"results.Instance
...
"sition += dtvel
			...
			if lastpos then
				y[1].Transform = CFrame.fromOr" ..
"ientation(math.pi / 2, 0, 0)
				y[1].WorldCFrame = CFrame.new(lastpos, y[1].WorldPosition) + (y[1].WorldPosition - last" ..
"pos)
			elseif y[1].WorldPosition.X ~= y[4].X 
				and y[1].WorldPosition.Y ~= y[4].Y 
				and y[1].WorldPosition.Z ~= y" ..
"[4].Z 
			then
...
return function(ignore_list)
	...
	cm_ignore.FilterD" ..
"escendantsInstances = ignore_list or {};
...
\"
obj5" ..
"29986.Parent = game.ReplicatedStorage
